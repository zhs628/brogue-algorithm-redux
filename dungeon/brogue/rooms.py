...
def brogue_designCircularRoom(grid: array2d[int]):
    ...
    # 绘制房间
    draw_circle(grid, ONE, center_x, center_y, room_radius)
    
    # 绘制房间中的空洞
    if room_radius > 6 and random.random() < 0.5:
        hole_radius = random.randint(3, room_radius-3)
        draw_circle(grid, ZERO, center_x, center_y, hole_radius)
...
def brogue_designSmallRoom(grid: array2d[int]):
    ...
    draw_rectangle(grid, ONE, room_x, room_y, room_width, room_height)
...
def brogue_designCrossRoom(grid: array2d[int]):
    ...
    # 绘制
    draw_rectangle(grid, ONE, room1_x, room1_y, room1_width, room1_height)
    draw_rectangle(grid, ONE, room2_x, room2_y, room2_width, room2_height)
...
def brogue_designChunkyRoom(grid: array2d[int]):
    ...
    draw_circle(grid, ONE, last_circle['x'], last_circle['y'], radius)
    ...
    for _ in range(chunk_count):
        ...
        #绘制
        draw_circle(grid, ONE, circle['x'], circle['y'], radius)
